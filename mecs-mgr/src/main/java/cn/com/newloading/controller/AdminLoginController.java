...
import cn.com.newloading.service.AdminLoginService;
import cn.com.newloading.utils.IsNullUtil;
//import cn.com.newloading.utils.RsaUtils;
...
public class AdminLoginController {
	...
	public JSONObject changePsd(String oldPsd,String newPsd,HttpSession session,HttpServletResponse resp) {
		...
		//把旧密码和新密码都用md5加密
		String bo=loginService.changPsd(oldPsd, newPsd, session);
  ...
  public String doLogout(HttpSession sess){
	...
	public JSONObject adminLogin(String keyCode, String adminAcc, String adminPswd, HttpServletRequest req) throws Exception {
		...
		if(notNull) {
			...
			/*
			 * if(codek == null || "".equals(codek)) { obj.put("loginback","codeErr");
			 * return obj; }
			 */
			...
			//if(codek.equalsIgnoreCase(keyCode)) {
				...
				String st=loginService.queryUser(adminAcc,adminPswd,session);	
