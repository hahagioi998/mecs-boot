...
public class BillingServiceImpl implements BillingService {
	...
	public JSONObject getUserInfoByCardNum(String cardNum) {
		...
		for (int i = 0; i < mecRecordList.size(); i++) {
			MecRecord mecRecord = mecRecordList.get(i);
	...
	public JSONObject sureBilling(List<String> comboList, String mrPrice, String cardNum) throws Exception {
		 ...
		 */
		String balance = billingMapper.getCardBalance(cardNum);
		if (Double.valueOf(mrPrice) > Double.valueOf(balance)) {
		...
		MecRecord mecRecord = new MecRecord(null, mrNum, cardId, comboName, mrPrice, null, null, null, null, "1", null,
				null, null, null);
		 ...
		 * 结算修改卡上余额
		 */
		Double newBalance = Double.valueOf(balance) - Double.valueOf(mrPrice);
		Integer changeCardBalance = billingMapper.changeCardBalance(cardNum, newBalance);

		/*
		 * 体检记录修改状态
		 */
		Integer changeMecRecordState = billingMapper.changeMecRecordState(mecRecord.getMrId());

		/*
		 * 修改项目记录表状态
		 */
		Integer changeItemsRecordState = billingMapper.changeItemsRecordState(mecRecord.getMrId());
